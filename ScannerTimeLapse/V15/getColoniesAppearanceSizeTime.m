function [ColoniesIndices, AppearanceTime, lbSizeTime, ubSizeTime] = getColoniesAppearanceSizeTime(FileDir, lb, ub)
%% [ColoniesIndices, AppearanceTime, lbSizeTime, ubSizeTime] = 
%       getColoniesAppearanceSizeTime(FileDir)
% -------------------------------------------------------------------------
...
AppearanceTime = zeros(length(ColoniesIndices),1);
lbSizeTime     = zeros(length(ColoniesIndices),1);
ubSizeTime     = zeros(length(ColoniesIndices),1);
for k=1:length(ColoniesIndices)
    ...
    lbSizeTime(k) = TimeAxis(lbIndex);
    ubSizeTime(k) = TimeAxis(ubIndex);
    
    AppearenceIndex = find(VecArea(NotCloseToBorder(k),:),1);
    AppearanceTime(k) = TimeAxis(AppearenceIndex);
end
